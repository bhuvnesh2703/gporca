	...
	)
{
	...
	DrgPimdobj *pdrgpmdobj = parse_handler_dxl->Pdrgpmdobj();
	DrgPsysid *pdrgpsysid = parse_handler_dxl->GetSystemIdArray();
	CDXLNode *pdxlnPlan = parse_handler_dxl->GetPlanDXLRoot();
	...
	)
{
	...
	GPOS_ASSERT(NULL != pdxlmd->Pdrgpmdobj() && NULL != pdxlmd->GetSystemIdArray() && "No metadata found in Minidump");
	GPOS_ASSERT(NULL != optimizer_config);
