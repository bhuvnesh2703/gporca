	...
	)
{
	...
	OpenInternal(file_path, O_CREAT | O_WRONLY | O_RDONLY | O_TRUNC, permission_bits);

	GPOS_ASSERT(0 == ioutils::UllFileSize(file_path));
	...
	)
{
	...
	while (0 < bytes_left_to_write)
	{
		...
		GPOS_CHECK_SIM_IO_ERR(&current_byte, ioutils::IWrite(IFileDescr(), read_buffer, bytes_left_to_write));
