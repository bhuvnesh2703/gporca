...
namespace gpdxl
{
	...
	class CDXLUtils 
	{
		...
		public:
			...
			static 
			DrgPimdobj *ParseDXLToIMDObjectArray
				(
				IMemoryPool *,
				const CHAR *dxl_string,
				const CHAR *xsd_file_path
			...
			static 
			DrgPdxlstatsderrel *ParseDXLToStatsDerivedRelArray
				(
				IMemoryPool *,
				const CHAR *dxl_string,
				const CHAR *xsd_file_path
			...
			static
			DrgPstats *ParseDXLToOptimizerStatisticObjArray
				(
				IMemoryPool *memory_pool,
			...
			static
			DrgPbucket *ParseDXLToBucketsArray
				(
				IMemoryPool *memory_pool,
				CMDAccessor *md_accessor,
			...
			static
			void DebugPrintMDIdArray(IOstream &os, DrgPmdid *pdrgpmdid);
#endif
	};
		...
		)
	{
		...
		for (ULONG ul = 0; ul < length; ul++)
		{
			...
			{
				string_var->AppendFormat(GPOS_WSZ_LIT("%d%ls"), value, CDXLTokens::PstrToken(EdxltokenComma)->Wsz());
			}
		}
