...
CDXLUtilsTest::EresUnittest_SerializePlan()
{
	...
	CDXLNode *pdxln = CDXLUtils::GetPlanDXLNode(memory_pool, dxl_string, NULL /*xsd_file_path*/, &plan_id, &plan_space_size);
	...
	for (ULONG ulHeaders = 0; ulHeaders < GPOS_ARRAY_SIZE(rgfHeaders); ulHeaders++)
	{
		...
		{
			oss << "Headers: " << rgfHeaders[ulHeaders] << ", indentation: " << rgfIndentation[ulIndent] << std::endl;
			CDXLUtils::SerializePlan(memory_pool, oss, pdxln, plan_id, plan_space_size, rgfHeaders[ulHeaders], rgfIndentation[ulIndent]);
			oss << std::endl;
		}
	}
...
CDXLUtilsTest::EresUnittest_Encoding()
{
	...
	XMLCh *pxmlch = GPOS_NEW_ARRAY(memory_pool, XMLCh, outputLength + 1);
