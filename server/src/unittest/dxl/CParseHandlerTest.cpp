...
CParseHandlerTest::EresUnittest_ErrSAXParseException()
{
	...
	// read DXL file
	CHAR *dxl_string = CDXLUtils::Read(pmp, m_rgszXerceTestFileNames[0]);
	
	// function call should throw an exception
	ULLONG plan_id = ULLONG_MAX;
	ULLONG plan_space_size = ULLONG_MAX;
	(void) CDXLUtils::GetPlanDXLNode(pmp, dxl_string, CTestUtils::m_szXSDPath, &plan_id, &plan_space_size);
	...
	)
{
	...
	// read DXL file
	CHAR *dxl_string = CDXLUtils::Read(pmp, dxl_filename);
	...
	ULLONG plan_id = ULLONG_MAX;
	ULLONG plan_space_size = ULLONG_MAX;
	CDXLNode *pdxlnRoot = CDXLUtils::GetPlanDXLNode(pmp, dxl_string, szValidationPath, &plan_id, &plan_space_size);
	...
	CDXLUtils::SerializePlan(pmp, osPlan, pdxlnRoot, plan_id, plan_space_size, true /*fSerializeHeaderFooter*/, true /*indentation*/);
	...
	)
{
	...
	// the root of the parsed DXL tree
	CQueryToDXLResult *pq2dxlresult = CDXLUtils::ParseQueryToQueryDXLTree(pmp, dxl_string, szValidationPath);
	...
	CDXLUtils::SerializeQuery(pmp, osQuery, pdxlnRoot, pdrgpdxln, pdrgpdxlnCTE, true /*fSerializeHeaderFooter*/, true /*indentation*/);
	...
	)
{
	...
	DrgPimdobj *pdrgpmdobj = CDXLUtils::ParseDXLToIMDObjectArray(pmp, dxl_string, szValidationPath);
	...
	CWStringDynamic *pstr = CDXLUtils::SerializeMetadata(pmp, pdrgpmdobj, true /*fSerializeHeaderFooter*/, true /*indentation*/);
	...
	)
{
	...
	CMDRequest *pmdr = CDXLUtils::ParseDXLToMDRequest(pmp, dxl_string, szValidationPath);
	
	GPOS_ASSERT(NULL != pmdr);
	
	GPOS_CHECK_ABORT;

	CDXLUtils::SerializeMDRequest(pmp, pmdr, oss, true /*fSerializeHeaderFooter*/, true /*indentation*/);
	...
	)
{
	...
	DrgPdxlstatsderrel *dxl_derived_rel_stats_array = CDXLUtils::ParseDXLToStatsDerivedRelArray(pmp, dxl_string, szValidationPath);
	DrgPstats *pdrgpstat = CDXLUtils::ParseDXLToOptimizerStatisticObjArray
								(
								pmp,
								&mda,
	...
	)
{
	...
	// the root of the parsed DXL tree
	CDXLNode *pdxlnRoot = CDXLUtils::ParseDXLToScalarExprDXLNode(pmp, dxl_string, szValidationPath);
	GPOS_CHECK_ABORT;

	CWStringDynamic str(pmp);
	COstreamString oss(&str);
	oss << "Serializing parsed tree" << std::endl;
	CWStringDynamic *pstr = CDXLUtils::SerializeScalarExpr(pmp, pdxlnRoot, true /*fSerializeHeaderFooter*/, true /*indentation*/);
	GPOS_CHECK_ABORT;
